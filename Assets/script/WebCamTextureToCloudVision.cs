	...
	void Sample_OnAnnotateImageResponses(AnnotateImageResponses responses) {
		if (responses.responses.Count > 0) {
			...
			if (responses.responses[0].textAnnotations != null && responses.responses[0].textAnnotations.Count > 0) {
				Debug.Log("joyLikelihood: " + responses.responses[0].textAnnotations[0].description);

				int stringLength = responses.responses [0].textAnnotations [0].description.Length;
				for (int i = 0; i < stringLength; i++) {
					Debug.Log (responses.responses [0].textAnnotations [0].description.Substring (i, 1));
					for (int j = 0; j < hiragana_array.Length; j++) {
						// Substringは文字列から抜き出す
						if ( hiragana_array[j] ==  responses.responses [0].textAnnotations [0].description.Substring (i, 1) ){
							Debug.Log(j);
							//"あ"は0番目
							//ひらがなごとの処理　順番
							// あとでコメントアウトから戻してね　GameObject.Instantiate( hiraganaCharacter[j] );
